...
## The draft model: how AI output becomes a client record

Every piece of content that the AI generates — session notes, transcripts, quick
memos, voice observations — follows the same three-step path before it can enter
a client's clinical record:

1. **Draft saved privately.** The AI creates a structured draft (themes, next
   steps, key quotes, or a verbatim transcript). The draft is encrypted and saved
   privately against the therapist's account. It is not yet part of the client's
   clinical record and is not visible to the client.

2. **Therapist reviews.** You read the draft on the session page or from the
   client timeline. You can edit freely — amend sentences, delete irrelevant
   content, or add context the recording missed.

3. **Therapist approves.** When you click **Confirm**, the reviewed draft is
   committed to the client's clinical record. Only confirmed content appears in
   the client's timeline, feeds the vector index, or is included in exports.

This applies to all AI-assisted content flows:
- Session summaries from uploaded recordings
- Voice memo drafts from quick observations ("Capture a thought")
- Transcript text before editing

Nothing generated by the AI enters a client record autonomously. Every item
waits in draft state until you approve it.
...
**Q: Is the AI draft saved automatically to the client record?**
A: No. The AI generates a private draft that only you can see. The draft enters
the client's clinical record only after you review and explicitly confirm it.
Nothing is committed to the record without your approval — not the session summary,
not the transcript, and not voice memo observations.

**Q: What happens to a draft if I close the browser before confirming?**
A: The draft is stored encrypted on the server. You will find it in the pending
state next time you open the session page. It never disappears silently and never
auto-publishes without your action.

**Q: Can a client see a draft before I approve it?**
A: No. Drafts are private to the therapist. Clients only see content that you
have explicitly confirmed and that their consent permits.
