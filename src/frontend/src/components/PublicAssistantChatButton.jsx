...
const INTRO_SEEN_KEY = 'public_assistant_intro_seen';

/**
 * Floating Action Button for anonymous landing page chat.
 * Renders on the landing page for unregistered visitors.
 */
export default function PublicAssistantChatButton() {
  const { t } = useTranslation();
  const isOpen = usePublicAssistantStore(s => s.isOpen);
  const togglePanel = usePublicAssistantStore(s => s.togglePanel);
    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {showTooltip && (
        <div className="absolute bottom-full right-0 mb-2 px-3 py-1.5 bg-gray-800 text-white text-sm rounded-lg shadow-lg whitespace-nowrap pointer-events-none">
          {t('publicChat.tooltip', 'Ask us anything!')}
      ...
      <button
        ...
        aria-label={t('publicChat.tooltip', 'Ask us anything!')}
      >
        {/* Chat bubble icon */}
